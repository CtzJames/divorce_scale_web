...
本项目已从单一“离婚力量表网页端”扩展为双量表网页端项目，包含公开前台测评页面和内部后台 MVP 两部分。

- 公开前台用于用户在线测评、查看结果页、提交联系方式、导出简版结果图并写入 Supabase。
- 当前公开前台包含离婚力量表 `/` 与配偶高自恋风险量表 `/narcissism-risk`。
- 两套量表均写入 Supabase `assessment_results`。
- 内部后台 MVP 位于 `apps/admin-mvp/`，两套量表共用同一后台，用于线索管理、单条详情查看、跟进字段维护、批量删除、CSV 导出、单条详细报告预览与导出。
- 当前仓库采用“根目录前台项目 + `apps/admin-mvp/` 后台子项目”的同仓库独立子项目路线。

当前项目已完成两套量表前台主流程、两套量表后台线索管理、两套量表单条详细报告、A4 网页端报告、移动端长图报告、PNG 导出与 PDF 导出的阶段性闭环，并已完成第 6-A 全链路构建与人工回归验收。当前 EdgeOne 海外区后台部署仅用于开发者线上验收、临时演示和技术验证，不是正式上线环境。
...
- 离婚力量表 `/`：在线测评、系统分流题前置、普通题随机、动态满分计算、结果页、三分流留资、简版 PNG 导出、Supabase 写库。
- 配偶高自恋特质与高冲突婚姻风险自测量表 `/narcissism-risk`：40 题测评、5 维评分、反向计分、NA / 与本人情况无关处理、四档风险等级、高风险触发题、五维雷达图、三分流留资、简版 PNG 导出、Supabase 写库。
- 两套量表均写入 `assessment_results`。
- 两套量表均写入联系方式字段 `contact_name`、`contact_phone`、`contact_wechat`。
- 两套量表均写入 `dimension_scores`。
...
后台单条详细报告模块当前位于 `/leads/[id]/report`。离婚力量表与配偶高自恋风险量表均已实现：
...
后台单条详细报告页当前支持两种预览形态和两类导出能力。离婚力量表与配偶高自恋风险量表均支持以下能力。
...
- 分别完成离婚力量表 `/` 与配偶高自恋风险量表 `/narcissism-risk` 的测评流程。
- 确认离婚力量表系统分流题、普通题随机、自动下一题正常。
- 确认新量表 40 题、反向计分、NA 处理、高风险触发题正常。
- 确认两套量表结果页分级、维度雷达图正常。
...
- 登录 `/login`。
- 打开 `/leads` 列表。
- 验证搜索、筛选、时间区间筛选、量表类型筛选。
- 验证结果等级筛选按量表类型区分。
- 验证 CSV 导出。
- 谨慎使用测试记录验证批量勾选删除。
- 分别打开两套量表记录的 `/leads/[id]` 详情页。
- 保存详情页后台字段与服务类型。
- 分别打开两套量表记录的 `/leads/[id]/report`。
- 预览 A4 网页端报告。
- 预览移动端长图报告。
- 导出 A4 PNG。
- 导出移动端 PNG。
...
- `docs/配偶高自恋特质量表后台与数据库适配方案V1.md`
- `docs/配偶高自恋特质与高冲突婚姻风险自测量表解读文案主档_V1.md`
